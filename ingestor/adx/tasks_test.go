...
func TestRecentCompletedOperationsNotRemoved(t *testing.T) {
	// This test reproduces the issue where completed operations that have fallen out of
	// Kusto's operations list but are recent (< 25 hours) are not being removed
	...
	// Create a summary rule with a recent async operation (should be completed but not old enough for stale removal)
	ruleName := "test-rule"
	operationId := "recent-completed-op"
	rule := &v1.SummaryRule{
		ObjectMeta: metav1.ObjectMeta{
			Name: ruleName,
		},
	...
	// Add a recent async operation (1 hour ago, definitely not older than 25 hours)
	// End time is 10 minutes ago to be past the 5-minute threshold
	recentTime := time.Now().Add(-1 * time.Hour)
	endTime := time.Now().Add(-10 * time.Minute)
	rule.SetAsyncOperation(v1.AsyncOperation{
		OperationId: operationId,
		StartTime:   recentTime.Format(time.RFC3339Nano),
		EndTime:     endTime.Format(time.RFC3339Nano),
	})

	// Set the last execution time to prevent new rule submission
	...
	// Set the GetOperations function to return an empty list (simulating completed operation no longer in list)
	task.GetOperations = func(ctx context.Context) ([]AsyncOperationStatus, error) {
		return []AsyncOperationStatus{}, nil
	}

	// Mock the SubmitRule function to avoid creating new operations for this test
	task.SubmitRule = func(ctx context.Context, rule v1.SummaryRule, startTime, endTime string) (string, error) {
		return "", fmt.Errorf("should not submit new rule in this test")
	}
	...
	// With the fix: the recent completed operation should be removed
	// because it has both start and end times and is not in Kusto's operations list
	require.Len(t, mockHandler.updatedObjects, 1, "Rule should have been updated")
	updatedRule, ok := mockHandler.updatedObjects[0].(*v1.SummaryRule)
	require.True(t, ok, "Updated object should be a SummaryRule")
	asyncOps := updatedRule.GetAsyncOperations()

	// After fix: operation should be removed
	require.Len(t, asyncOps, 0, "Completed operation with end time should be removed even if recent")
}
