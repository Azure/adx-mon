...
// TestMainBranchBugDemo demonstrates the specific bug that existed in main branch:
// Failed operations with ShouldRetry=1 were not retried because they were removed
// before the retry logic could execute. This test would FAIL on main branch.
func TestMainBranchBugDemo(t *testing.T) {
	// Create a mock statement executor
	mockExecutor := &TestStatementExecutor{
		database: "testdb",
		endpoint: "http://test-endpoint",
	...
	// Create a summary rule that has already been executed recently
	// This prevents normal interval-based execution from triggering
	rule := &v1.SummaryRule{
		ObjectMeta: metav1.ObjectMeta{
			Name: "failed-rule",
		},
		Spec: v1.SummaryRuleSpec{
			...
			Table:    "TestTable",
			Interval: metav1.Duration{Duration: time.Hour},
			Body:     "TestQuery | where BadColumn == true", // This would cause a syntax error
		},
	}

	// Set last execution time to now so no new time-based executions happen
	rule.SetLastExecutionTime(time.Now().UTC())

	// Add a failed async operation that should be retried
	originalStartTime := "2025-06-20T10:00:00Z"
	originalEndTime := "2025-06-20T11:00:00Z"
	rule.SetAsyncOperation(v1.AsyncOperation{
		OperationId: "failed-operation-should-retry",
	...
	// Track all retry attempts - this is the key test
	...
	// Mock SubmitRule to track retry attempts
	task.SubmitRule = func(ctx context.Context, rule v1.SummaryRule, startTime, endTime string) (string, error) {
		newOpId := fmt.Sprintf("retry-op-%d", len(retryAttempts)+1)
		...
		t.Logf("Retry attempt: operationId=%s, startTime=%s, endTime=%s", newOpId, startTime, endTime)
		return newOpId, nil
	}

	// Mock GetOperations to return the failed operation with ShouldRetry=1
	task.GetOperations = func(ctx context.Context) ([]AsyncOperationStatus, error) {
		return []AsyncOperationStatus{
			{
				OperationId:   "failed-operation-should-retry",
				...
				ShouldRetry:   1,        // Should be retried
			},
		}, nil
	}

	// Mock GetOperationErrorDetail for better error messages
	task.GetOperationErrorDetail = func(ctx context.Context, operationId string) (string, error) {
		return "Semantic error: SEM0001: 'BadColumn' is not a recognized column", nil
	}
	...
	// CRITICAL TEST: Verify that the failed operation was retried
	// This would FAIL on main branch because failed operations were removed
	// before retry logic could execute
	require.NotEmpty(t, retryAttempts, "Failed operation with ShouldRetry=1 MUST be retried - this would FAIL on main branch")

	// Verify the retry was for the SAME time window that originally failed
	require.Len(t, retryAttempts, 1, "Should have exactly one retry attempt")
	retry := retryAttempts[0]
	require.Equal(t, originalStartTime, retry.startTime,
		"Retry should use the same start time as the original failed operation")
	require.Equal(t, originalEndTime, retry.endTime,
		"Retry should use the same end time as the original failed operation")

	// Verify that the rule status was updated with the detailed error
	require.NotEmpty(t, mockHandler.updatedObjects, "Rule status should be updated")
	updatedRule, ok := mockHandler.updatedObjects[0].(*v1.SummaryRule)
	require.True(t, ok, "Updated object should be a SummaryRule")

	condition := updatedRule.GetCondition()
	require.NotNil(t, condition, "Rule should have a condition")
	require.Equal(t, metav1.ConditionFalse, condition.Status, "Condition should indicate failure")

	// Verify detailed error message is included
	require.Contains(t, condition.Message, "Semantic error: SEM0001",
		"Error message should contain detailed Kusto error information")

	t.Logf("SUCCESS: This test demonstrates that the bug from main branch is fixed!")
	t.Logf("On main branch, this test would FAIL because:")
	t.Logf("  1. Failed operations were considered 'completed'")
	t.Logf("  2. Completed operations were removed before retry logic could run")
	t.Logf("  3. ShouldRetry=1 was ignored for failed operations")
}
