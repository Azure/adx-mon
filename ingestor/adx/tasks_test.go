...
func TestManagementCommandCriteriaExpression(t *testing.T) {
	...
	t.Run("skips execution when expression is false", func(t *testing.T) {
		...
		cmd := handler.updatedObjects[0].(*adxmonv1.ManagementCommand)
		crit := apimeta.FindStatusCondition(cmd.Status.Conditions, v1.ManagementCommandCriteriaMatch)
		require.NotNil(t, crit)
		require.Equal(t, metav1.ConditionFalse, crit.Status)
		require.Equal(t, v1.ReasonCriteriaNotMatched, crit.Reason)

		execCond := apimeta.FindStatusCondition(cmd.Status.Conditions, v1.ManagementCommandExecuted)
		require.NotNil(t, execCond)
		require.Equal(t, metav1.ConditionFalse, execCond.Status)
		require.Equal(t, v1.ReasonCriteriaNotMatched, execCond.Reason)
		require.Contains(t, execCond.Message, "criteria expression evaluated to false")

		owner := apimeta.FindStatusCondition(cmd.Status.Conditions, v1.ManagementCommandConditionOwner)
		require.NotNil(t, owner)
		require.Equal(t, metav1.ConditionFalse, owner.Status)
		require.Equal(t, v1.ReasonCriteriaNotMatched, owner.Reason)
	})
	...
	t.Run("records error when expression evaluation fails", func(t *testing.T) {
		...
		require.NotNil(t, crit)
		require.Equal(t, metav1.ConditionFalse, crit.Status)
		require.Equal(t, v1.ReasonCriteriaExpressionError, crit.Reason)
		require.Contains(t, crit.Message, "criteriaExpression evaluation failed")

		execCond := apimeta.FindStatusCondition(cmd.Status.Conditions, v1.ManagementCommandExecuted)
		require.NotNil(t, execCond)
		require.Equal(t, metav1.ConditionFalse, execCond.Status)
		require.Equal(t, v1.ReasonManagementCommandCriteriaError, execCond.Reason)
		require.Contains(t, execCond.Message, "criteriaExpression evaluation failed")

		owner := apimeta.FindStatusCondition(cmd.Status.Conditions, v1.ManagementCommandConditionOwner)
		require.NotNil(t, owner)
		require.Equal(t, metav1.ConditionFalse, owner.Status)
		require.Equal(t, v1.ReasonManagementCommandCriteriaError, owner.Reason)
		require.Contains(t, owner.Message, "criteriaExpression evaluation failed")
	})
}
