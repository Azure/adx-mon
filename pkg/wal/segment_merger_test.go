...
func TestNewMultiReader(t *testing.T) {
	...
	defer s1.Close()
	require.NoError(t, s1.Write(context.Background(), []byte("foo")))
	require.NoError(t, s1.Write(context.Background(), []byte("bar")))
	require.NoError(t, s1.Flush())

	s2, err := NewSegment(dir, "Db_Table")
	require.NoError(t, err)
	defer s2.Close()
	require.NoError(t, s2.Write(context.Background(), []byte("foo1")))
	require.NoError(t, s2.Write(context.Background(), []byte("bar1")))
