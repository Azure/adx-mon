...
The kubelet-discovery configuration enables pod discovery via the local kubelet API instead of the Kubernetes API server. When configured, the collector polls the kubelet's `/pods` endpoint to discover running pods on the node. This method of discovery is useful in large cluster scenarios where minimizing API server load is important. When the `[kubelet-discovery]` block exists, [Prometheus Scrape](#prometheus-scrape) and [Host Log](#host-log) will utilize this mechanism to discover pods.
...
```toml
# Optional: Kubelet-based pod discovery configuration.
# When present, uses kubelet polling instead of Kubernetes API watch.
# Reduces API server load.
[kubelet-discovery]
  # Kubelet host address. Defaults to node name from NODE_NAME env var, or '127.0.0.1' if not set
  host = '127.0.0.1'
  # Kubelet port. Defaults to 10250
  port = 10250
  # How often to poll the kubelet for pod updates (seconds). Defaults to 10
  poll-interval = 15
  # Request timeout for kubelet API calls (seconds). Defaults to 9
  request-timeout = 10
  # Path to service account token. Defaults to /var/run/secrets/kubernetes.io/serviceaccount/token
  token-path = '/var/run/secrets/kubernetes.io/serviceaccount/token'
  # Path to CA certificate for TLS verification. Defaults to /var/run/secrets/kubernetes.io/serviceaccount/ca.crt
