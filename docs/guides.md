...
| `Reconciled` | `True` (reason `KustoExecutionSucceeded`) | The Function body was applied successfully in Kusto. | No action needed. |
| `Reconciled` | `True` (reason `FunctionDeleted`) | The Function resource was finalized successfully. Deletion currently uses a no-op finalization flow and does not drop the underlying Kusto function. | Delete or update the Kusto function manually if cleanup is required. |
