...
// CalculateNextExecutionWindow calculates the next execution window for a MetricsExporter
// This follows the same pattern as SummaryRule.NextExecutionWindow
func (qe *QueryExecutor) CalculateNextExecutionWindow(lastExecutionTime *time.Time, interval time.Duration) (windowStartTime time.Time, windowEndTime time.Time) {
	if lastExecutionTime == nil {
		// First execution: start from current time aligned to interval boundary, going back one interval
		now := qe.clock.Now().UTC()
		// Align to interval boundary for consistency
		alignedNow := now.Truncate(interval)
		windowEndTime = alignedNow
		windowStartTime = windowEndTime.Add(-interval)
	} else {
		// Subsequent executions: start from where the last successful execution ended
		windowStartTime = *lastExecutionTime
		windowStartTime = windowStartTime.Truncate(interval)
		windowEndTime = windowStartTime.Add(interval)

		// Ensure we don't execute future windows
		now := qe.clock.Now().UTC().Truncate(time.Minute)
...
func (qe *QueryExecutor) ShouldExecuteQuery(lastExecutionTime *time.Time, interval time.Duration, lastTransitionTime time.Time, generation int64, observedGeneration int64) bool {
	// Always execute if generation changed (spec was updated)
	if observedGeneration != generation {
		return true
	}

	if lastExecutionTime != nil {
		// Time for next interval based on last successful execution
		return qe.clock.Since(*lastExecutionTime) >= interval
	}

	// First execution timing
	return qe.clock.Since(lastTransitionTime) >= interval
}
