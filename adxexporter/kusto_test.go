...
func TestQueryExecutor_CalculateNextExecutionWindow(t *testing.T) {
	fixedTime := time.Date(2023, 1, 1, 14, 30, 45, 0, time.UTC)
	fakeClock := NewFakeClock(fixedTime)

	mockClient := NewMockKustoExecutor("TestDB", "https://test.kusto.windows.net")
	executor := NewQueryExecutor(mockClient)
	executor.SetClock(fakeClock)
	...
	tests := []struct {
		...
		{
			name:              "first execution - 5 minute interval",
		...
		{
			name:              "first execution - 1 hour interval",
			lastExecutionTime: nil,
		...
		{
			name:              "subsequent execution",
			lastExecutionTime: &[]time.Time{time.Date(2023, 1, 1, 14, 0, 0, 0, time.UTC)}[0],
		...
		{
			name:              "subsequent execution - don't go into future",
			lastExecutionTime: &[]time.Time{time.Date(2023, 1, 1, 14, 0, 0, 0, time.UTC)}[0],
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			startTime, endTime := executor.CalculateNextExecutionWindow(tt.lastExecutionTime, tt.interval)
...
func TestQueryExecutor_ShouldExecuteQuery(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:                  "first execution - time passed",
			lastExecutionTime:     nil,
			interval:              5 * time.Minute,
			lastTransitionTime:    time.Date(2023, 1, 1, 14, 20, 0, 0, time.UTC), // 10 minutes ago
			generation:            1,
			observedGeneration:    1,
			expectedShouldExecute: true,
		},
		{
			name:                  "first execution - time not passed",
			lastExecutionTime:     nil,
			interval:              15 * time.Minute,
			lastTransitionTime:    time.Date(2023, 1, 1, 14, 20, 0, 0, time.UTC), // 10 minutes ago
			generation:            1,
		...
		{
			name:                  "subsequent execution - time passed",
			lastExecutionTime:     &[]time.Time{time.Date(2023, 1, 1, 14, 20, 0, 0, time.UTC)}[0], // 10 minutes ago
			interval:              5 * time.Minute,
			lastTransitionTime:    time.Date(2023, 1, 1, 14, 0, 0, 0, time.UTC),
			generation:            1,
			observedGeneration:    1,
			expectedShouldExecute: true,
		},
		{
			name:                  "subsequent execution - time not passed",
			lastExecutionTime:     &[]time.Time{time.Date(2023, 1, 1, 14, 26, 0, 0, time.UTC)}[0], // 4 minutes ago
			interval:              5 * time.Minute,
			lastTransitionTime:    time.Date(2023, 1, 1, 14, 0, 0, 0, time.UTC),
			generation:            1,
			observedGeneration:    1,
		...
		},
		{
			...
			lastExecutionTime:     &[]time.Time{time.Date(2023, 1, 1, 14, 29, 0, 0, time.UTC)}[0], // 1 minute ago
			interval:              5 * time.Minute,
			lastTransitionTime:    time.Date(2023, 1, 1, 14, 0, 0, 0, time.UTC),
			generation:            2,
			observedGeneration:    1,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := executor.ShouldExecuteQuery(
				tt.lastExecutionTime,
				tt.interval,
				tt.lastTransitionTime,
				tt.generation,
				tt.observedGeneration,
			)
			assert.Equal(t, tt.expectedShouldExecute, result)
		})
	}
}
