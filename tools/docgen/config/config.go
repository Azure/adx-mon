...
func main() {
	...
	// Render to buffer first so we can inject the warning comment
	var buf bytes.Buffer
	if err := pageTemplate.Execute(&buf, contents); err != nil {
		panic(err)
	}

	// Parse the rendered content to inject comment after front matter
	renderedContent := buf.String()
	frontMatterEnd := "---\n"
	if idx := bytes.Index(buf.Bytes()[4:], []byte(frontMatterEnd)); idx != -1 {
		// Found closing front matter, inject comment after it
		insertPos := idx + 4 + len(frontMatterEnd)
		renderedContent = renderedContent[:insertPos] + "\n<!-- Generated by tools/docgen/config - DO NOT EDIT -->\n" + renderedContent[insertPos:]
	}
	...
	if _, err := output.WriteString(renderedContent); err != nil {
		panic(err)
	}
}
